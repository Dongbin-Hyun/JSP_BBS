...
<body>
	<%
		...
		Bbs bbs = new BbsDAO().getBbs(bbsID);//현재 넘어온 글의 작성자가 맞는지 확인
		if (!userID.equals(bbs.getUserID())) {
			...
			if (request.getParameter("bbsTitle") == null || request.getParameter("bbsContent") == null || (request.getParameter("bbsTitle").equals("")) || request.getParameter("bbsContent").equals("")) {
				...
				script.println("</script>");
					...
					int result = bbsDAO.update(bbsID, request.getParameter("bbsTitle"), request.getParameter("bbsContent"));//글이 수정되도록 함
					if (result == -1) {
						PrintWriter script = response.getWriter();
						script.println("<script>");
